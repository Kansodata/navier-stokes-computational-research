...
Scientific validation consolidated PDF command: `python -m navier_stokes_research.cli --scientific-validation-report --pdf` -> `outputs/reports/scientific_validation_report.pdf` (human-readable scientific review artifact with visual summary, paginated validation matrix, conservative interpretation, warnings/gaps cards, up to 3 generated figures, and final artifact index; fail-closed when critical evidence produces overall `failed` status).
